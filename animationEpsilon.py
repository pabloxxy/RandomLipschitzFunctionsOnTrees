...
# ----------------------------------------------------------------------------------------------------------------
# ----- Make an animation: distributions with mass on two points around 0 and epsilon in between and outside -----
# ----------------------------------------------------------------------------------------------------------------
...
eps_out = 40                   # number of epsilon points outside the mass 
fps = 30                       # frames per second in animation


# ----- Set the initial distribution -----
new_half = L_half + eps_out + 1 # New half‑length: original half + eps_out (epsilon points) + 1 (the zero at the end)
total_len = 2 * new_half + 1
z0 = [mp.mpf(0)] * total_len    # initialise with zeros

# Fill the distribution symmetrically
centre_idx = new_half # Indices run from -new_half ... +new_half, stored in a list with centre at index new_half.

# Set the mass points at ±L_half
a = (1 - epsilon * (2 * L_half - 1 + 2 * eps_out)) / 2 # Compute the mass value "a" so that the total probability sums to 1
z0[centre_idx - L_half] = a
z0[centre_idx + L_half] = a

# Set the interior epsilon points (from -L_half+1 to L_half-1, including centre)
for i in range(-(L_half-1), L_half):      # i from -(L_half-1) to L_half-1 inclusive
    if i == 0:
        # centre (distance 0)
        z0[centre_idx] = epsilon
    else:
        z0[centre_idx + i] = epsilon

# Set the new epsilon points outside the mass (distances L_half+1, ..., L_half+eps_out)
for i in range(1, eps_out + 1):
    # right side
    z0[centre_idx + L_half + i] = epsilon
    # left side
    z0[centre_idx - L_half - i] = epsilon

# The outermost points (distances L_half+eps_out+1) are already zero from initialisation.
# print(z0) # Debugging
...
y_max = 10**10                # cutoff for values that get too big 
def update(frame):
...
anim.save('animation.mp4', writer='ffmpeg', fps=fps, dpi=100)
plt.close(fig)  